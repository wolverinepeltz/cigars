...
Usage:
    ...
    python hilands_crawl.py --sales-only --min-pack 5
...
def api_raw_products(cat_id, delay):
...
def api_variation_rows(parent, delay):
    """Fetch each variation of a product as its own row."""
...
def main():
    ...
    if cats:
        ...
        if expand:
            print("Variation pass on. This is slow: one request per size.")
        if args.min_pack > 1:
        ...
        for cat_id, name, count in targets:
            ...
            for p in api_raw_products(cat_id, args.delay):
                ...
                if expand and variations:
                    for row in api_variation_rows(p, args.delay):
                        row["brand"] = name
                        if keep(row):
                            rows.append(row)
                            watch(row)
    ...
    big = sorted([r for r in on_sale if r["discount_pct"] >= args.alert_pct],
